...
# chrome_options.add_argument('--ignore-certificate-errors')  # 无头模式，不显示浏览器窗口
# 创建 ChromeDriver 实例
...
for i in range(2104,2106):
    ...
    try:
        ...
        try:
            ...
            driver.refresh()
            se_title = WebDriverWait(driver, 4).until(EC.presence_of_element_located((By.XPATH, '//*[@id="main-content"]/div[2]/h1')))
            sequence_title = se_title.text
            WebDriverWait(driver, 4).until(EC.presence_of_element_located((By.ID, 'giraffe-analyze-frame')))
            driver.switch_to.frame('giraffe-analyze-frame')
            se_data = WebDriverWait(driver, 4).until(EC.presence_of_element_located((By.XPATH, '/html/body/div[1]/div[2]/div[2]/div/div[2]/textarea')))
            ...
            list.append(i)
            print(datetime.now().strftime("%m-%d %H:%M:%S") + ": " + "未找到基因序列信息: " + str(i) + "   请求超时")
            continue
    try:
        # time.sleep(0.5)
        se_title = WebDriverWait(driver, 4).until(EC.presence_of_element_located((By.XPATH, '//*[@id="main-content"]/div[2]/h1')))
...
for i in list:
    ...
    try:
        ...
        try:
            ...
            with open(file_path, "w", encoding='utf-8') as file:
                file.write(sequence_data)
            print(datetime.now().strftime("%m-%d %H:%M:%S") + ": " + "二次查询: 找到基因序列信息: " + str(i) + "  " + sequence_title)
            continue
        except TimeoutException:
            print(datetime.now().strftime("%m-%d %H:%M:%S") + ": " + "二次查询: 未找到基因序列信息: " + str(i) + "   请求超时")
            continue
    try:
        # time.sleep(0.5)
        se_title = WebDriverWait(driver, 2).until(EC.presence_of_element_located((By.XPATH, '//*[@id="main-content"]/div[2]/h1')))
        ...
        print(datetime.now().strftime("%m-%d %H:%M:%S") + ": " + "二次查询: 未找到基因序列信息: " + str(i))
        # time.sleep(1)
        # driver.save_screenshot(files_path + str(i) + ".png")
    except TimeoutException as e:
        print(datetime.now().strftime("%m-%d %H:%M:%S") + ": " + "二次查询: 未找到基因序列信息: " + str(i))
